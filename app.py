...
with st.sidebar:
    ...
    if api_key:
        ...
        st.caption("API 키는 console.anthropic.com에서 발급받을 수 있습니다.")
    st.divider()
    ...
    st.divider()
    st.markdown("**사용 방법**")
    st.markdown("1. 질문 입력 후 AI 응답 받기")
    st.markdown("2. A/B 응답 자동 생성 확인")
    st.markdown("3. 위험 탐지 결과 확인")
    st.markdown("4. 체크리스트 검토 후 수정 방향 입력")
    st.markdown("5. C응답 생성 및 A/B/C 비교")
...
def get_ai_response(question, mode="basic", review_notes=""):
    ...
    if mode == "basic":
        prompt = f"""질문의 요구 조건을 모두 지켜 답하라. 별도의 자기검토 과정은 수행하지 말라.
...
def evaluate_response(response):
    scores = {
        "정확성": 0, "핵심요소반영": 0, "엄밀성": 0,
        "신중성": 0, "형식충족": 0, "표현완성도": 0
    }
    if len(response) > 100:
        scores["정확성"] = 3
    ...
    scores["표현완성도"] = 1 if (response.strip() and not response.endswith('...')) else 0
...
def make_comparison_chart(scores_a, scores_b, scores_c):
    ...
    def normalize(scores):
        return [scores[c] / max_scores[i] * 10 for i, c in enumerate(categories)]
    fig = go.Figure()
    fig.add_trace(go.Bar(name='A: 기본 응답', x=categories, y=normalize(scores_a), marker_color='#6baed6'))
    fig.add_trace(go.Bar(name='B: 자기검토 응답', x=categories, y=normalize(scores_b), marker_color='#74c476'))
    fig.add_trace(go.Bar(name='C: HITL 응답', x=categories, y=normalize(scores_c), marker_color='#fd8d3c'))
    fig.update_layout(
        barmode='group',
...
for key, default in {
    ...
    'risks': [], 'risk_score': 0, 'step': 1, 'question': ''
}.items():
...
if st.button("🤖 AI 응답 받기 (A·B 동시 생성)", type="primary", disabled=not api_key):
    if not question.strip():
        ...
        with col_p2:
            with st.spinner("B조건 (자기검토 응답) 생성 중..."):
                st.session_state.response_b = get_ai_response(question, "self_review")
        st.session_state.risks, st.session_state.risk_score = detect_risk(st.session_state.response_a)
...
if st.session_state.step >= 2 and st.session_state.response_a:
    ...
    with col1:
        ...
        scores_a_p, total_a_p = evaluate_response(st.session_state.response_a)
        st.caption(f"자동 평가 점수: {total_a_p}/10")
    with col2:
        st.subheader("🟢 B: 자기검토 응답")
        st.success(st.session_state.response_b)
        scores_b_p, total_b_p = evaluate_response(st.session_state.response_b)
        st.caption(f"자동 평가 점수: {total_b_p}/10")
    ...
    review_notes = st.text_area(
        ...
        placeholder="예: A응답의 수식 표현은 너무 전문적입니다. 수식 없이 말로 풀어서 설명하고, 마지막 문장을 더 간결하게 마무리해주세요.",
        height=100
    )
...
if st.session_state.step >= 3 and st.session_state.response_c:
    ...
    with col6:
        ...
        st.metric("총점", f"{total_b}/10", delta=f"+{delta_b}" if delta_b > 0 else str(delta_b))
    with col7:
        st.subheader("🟠 C: HITL 응답")
        st.warning(st.session_state.response_c)
        delta_c = total_c - total_a
        st.metric("총점", f"{total_c}/10", delta=f"+{delta_c}" if delta_c > 0 else str(delta_c))
    ...
    result_data = {
        ...
        "A_총점": [total_a], "B_총점": [total_b], "C_총점": [total_c],
        "B_향상": [total_b - total_a], "C_향상": [total_c - total_a]
    }
    ...
    if st.button("🔁 새 질문으로 다시 시작"):
        for key in ['response_a', 'response_b', 'response_c', 'risks', 'question']:
